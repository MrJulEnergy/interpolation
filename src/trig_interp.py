...
def trigonometric_interpolation(points: np.array) -> np.array:
    ...
    for dim in range(d):
        z = points[dim, :]
        a = np.array([2 / (n + 1) * np.sum([z[j] * np.cos(j * t_i[k]) for j in range(n + 1)]) for k in range(0, m + 2)])
        b = np.array([2 / (n + 1) * np.sum([z[j] * np.sin(j * t_i[k]) for j in range(n + 1)]) for k in range(1, m + 1)])

        f1 = a[0] / 2
        f2 = np.sum([a[k] * np.cos(k * t) + b[k - 1] * np.sin(k * t) for k in range(1, m + 1)], axis=0)
